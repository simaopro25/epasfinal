...
// setFieldUpload($field,$path,$size,$method) - Change field to upload type for file uploads. $path is the directory where files will be uploaded and size is the default size of a image or icon, 
// 												and method is the method to upload image (POST - default or PUT).
...
public function makeAlist($withForms=true){
	...
	if ($withForms){												//DUVIDAS NESTA ESCOLHA
		//echo "<br>com forms";
		//$formAU=$this->prepareEditNewForm();  					//OPÇÃO a
		if ($this->countFieldsForAction() > 20) {					//OPÇÃO b
	...
	foreach($this->camposLista as $campo){
		...
		if ($campo['ver']==1){
			...
			if ($campo['Type']=="file"){
				...
				//$pos[$i]['pre'] = '<a href="' . $aux . "/";													//dUVIDAS AQUI COM A FORMA COMO APRESENTO O UPLOAD
		...
		foreach($this->camposLista as $campoaux){
			...
			if ($this->camposLista[$i]['Type']=="file"){
				if (isset($_REQUEST[$nomeCampo]) && ($_REQUEST[$nomeCampo]!="")){
					$path=$this->camposLista[$i]["Path"];
					$path=str_replace($_SERVER['DOCUMENT_ROOT'],"",$path);
					$this->camposLista[$i]["valor"]=$path.$_REQUEST[$nomeCampo];
				...
				if ($campoaux['csv']==1){	
					...
					if ($registo[$j]!=""){
						...
						if ($campoaux['Type']=="date"){
							...
							$date=str_replace(".","-",$date);
							$parts=explode("-",$date);
							//if (sizeof($part[0])<4){													//DUVIDA AQUI
							if (sizeof($parts) < 4) {	
	...
	?>
		<script>
			...
			<?php
				...
				if ($this->metodoUpload=="POST"){
					$uf->JSUploadFileUsingPOST("EndPoint");
				}else{
					$uf->JSUploadFileUsingPUT("EndPoint");
				}
		...
		switch ($aux) {
			case "fil":
				...
				foreach($html->find('#fileL') as $e)
				 ...
				 foreach($html->find('#fileI') as $e)
				 	$e->id="txt2" . $field['Field'];   ;
...
//###################################################################################################################################	
	...
	* @param size    The default sizo of a image or icon
     * 
	...
	*/
	public function setFieldUpload($field,$path, $size=30, $method="POST"){
		$i=0;
		//echo "<br> campo=$campo accao=$accao e valor=$valor";
		$this->metodoUpload=$method;
		foreach($this->camposLista as $campoaux){
			if ($campoaux['Field']==$field){
				//echo "entrie";
