...
class ControllerLoginSimples{
  ...
  public function getUserByUserCode($user, $code, $json = true)  {
    ...
    $p['code'] = md5($code);
    //sprint_r($p);
    $user = $this->database->getData("SELECT `id`, `nome`, `localizacao`, `pais`, `foto`, `telefone`, `email`, `TipoEscola`, `PaginaWeb`, `sobreNos`, `utilizador`, `passw`, 
                                          `pessoaContacto`, `motivacao`, `confirmada`, `latitude`, `longitude`, code FROM `Escolas` WHERE `email`=:email and code=:code and `confirmada`=1", $p);
    if ($json) {
      echo json_encode($user);
    } else {
      return $user;
   ...
   public function getUserbyEmail($email, $json = true)  {
    $p['email'] = $email;
    $user = $this->database->getData("SELECT `id`, `nome`, `localizacao`, `pais`, `foto`, `telefone`, `email`, `TipoEscola`, `PaginaWeb`, `sobreNos`, `utilizador`, `passw`, 
                                          `pessoaContacto`, `motivacao`, `confirmada`, `latitude`, `longitude` FROM `Escolas` WHERE `email`=:email and `confirmada`=1", $p);
    if ($json) {
      echo json_encode($user);
    } else {
      return $user;
  ...
  public function   sendRecoveryEmail($user) {
    ...
    $userUpd = $this->database->getData("UPDATE `Escolas` SET `code`=:code WHERE `email`=:email", $p);
    //print_r($user);
...
public function recoverLoginS2()  {
  ...
  public function updatePass() {
      ...
      //print_r($p);
        $resp = $this->database->setData("UPDATE `Escolas` SET `passw`=:passw  WHERE `email`=:email and `code`=:code", $p);
