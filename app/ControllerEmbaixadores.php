...
class ControllerEmbaixadores {
    ...
    //Contar 
    public function contarSeniores() {
        $dados = $this->database->getData("SELECT COUNT(id) AS total FROM Embaixadores WHERE tipo = 'Senior' AND ativo = 1");
        echo json_encode($dados, JSON_UNESCAPED_UNICODE);
}

// Contar Juniores Ativos
    public function contarJuniores() {
        $dados = $this->database->getData("SELECT COUNT(id) AS total FROM Embaixadores WHERE tipo = 'Junior' AND ativo = 1");
        echo json_encode($dados, JSON_UNESCAPED_UNICODE);
}
